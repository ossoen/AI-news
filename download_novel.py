...
CATALOG_URL = "https://www.66kxs.net/book/3993/3993369/"
HEADERS = {
...
def download_one(chapter, completed_set):
    url = chapter["url"]
...
def main():
    ...
    for i, ch in enumerate(todo, 1):
        result = download_one(ch, completed)
        if result:
            results.append(result)
            if result.get("failed"):
                failed.append(ch["url"])
            else:
                completed.add(ch["url"])
        ...
        if i % 10 == 0:
            ...
            remaining = (len(todo) - i) / speed if speed > 0 else 0
            print(f"  进度: {i}/{len(todo)} | 速度: {speed:.1f}章/秒 | 预计剩余: {remaining/60:.1f}分钟")

        time.sleep(0.15)  # 轻微延迟避免被封
    ...
    with open(OUTPUT_FILE, "a", encoding="utf-8") as f:
        for ch in chapters:
            if ch["url"] in content_map and ch["url"] not in existing:
